...
class Number:
    ...
    def as_bits_lsb_first(self):
        ...
        If the length of the Number is not None then the list is either extended or truncated
        to the given length.
...
class ContinuousAssign:
    ...
    def __repr__(self):
        return "ContinuousAssign({})" \
            .format(", ".join(("{} = {}".format(left, right) for left, right in self.assignments)))
...
class VerilogTransformer(Transformer):
    ...
    def concatenation(self, left) -> Concatenation:
        result = []
        for x in left:
