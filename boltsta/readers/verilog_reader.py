...
# Importing a Verilog parser from an external module
...
def extract_input_output_pins_of_cells(ast):
    ...
    try:
        ...
        for inst in ast.modules[0].module_instances:
            ...
            output_pins.append(a[-1])  # Append the last port name as an output pin
            # Loop through all but the last port to consider them as input pins
            for i in range(len(a) - 1):  # Loop through all ports except the last
                input_pins.append(a[i])  # Append the port name as an input pin
                # Modify the input pin name by adding the output pin as a prefix
                a[i] = a[-1] + "_" + a[i]  # Combine output pin name with input pin name
...
def extract_mod_input_pins(ast):
    """
    Extracts the modified input pins of the design cells by adding the corresponding output pin as a prefix.
    This is done again to remove the output 'Q' from these modified inputs and deal with the correct version of the cells' input pins.
    ...
    try:
        ...
        for inst in ast.modules[0].module_instances:
            ...
            for port, node in inst.ports.items():
                ...
                )  # Map port to instance and module
...
def extract_unique_internal_nodes(ast, mod_input_pins, port_to_node_to_instance):
    """
    Extracts the unique internal connections of the verilog design. These connections are to be used to build
    the graph representing the verilog design. Also builds an important dict (port_to_node_to_instance) which binds ports
    to the design cells to make searching for nodes efficient.
    ...
    try:
        ...
        if not isinstance(mod_input_pins, list):
            raise ValueError(
                "Modified input pins must be a list."
        ...
        # Get the internal nodes by excluding port names from the signal names
        internal_nodes = set(all_signals) - set(
            ast.modules[0].port_list
        )  # Internal nodes are signals not in the port list
                ...
                for conn2, module2, port2 in connections[i + 1:]:
                    ...
                    if port2 == "Q":
                        internal_connections.append(
                            [conn2, module2, conn1, module1, port1]
                        )  # Handle special case for 'Q'
                    else:
                        internal_connections.append(
                            [conn1, module1, conn2, module2, port2]
                        )  # Add the connection

        # Return the list of internal connections and the port-to-instance mapping
        return internal_connections  # Return internal connections and port-to-instance mapping
...
def find_partial_match(nets, input_list):
    """
    A utility function used to find partial matches between two given lists and returns a new list
    with the matching elements.
    ...
    try:
        ...
        if not isinstance(nets, (list, type({}.keys()))) or not isinstance(input_list, list):
            raise ValueError(
                "Nets must be list or dict_keys and input_list must be list."
